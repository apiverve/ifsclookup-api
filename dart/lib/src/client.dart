...
class IfsclookupValidationException implements Exception {
  final List<String> errors;

  IfsclookupValidationException(this.errors);

  @override
  String toString() => 'IfsclookupValidationException: ${errors.join("; ")}';
}
...
/// IFSC Lookup API client.
///
/// For more information, visit: https://apiverve.com/marketplace/ifsclookup?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [ifsc] (required) - The IFSC code to get information about
class IfsclookupClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'ifsc': ValidationRule(type: 'string', required: true),
  };

  IfsclookupClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/ifsclookup',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [IfsclookupValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw IfsclookupValidationException(errors);
    }
  }

  /// Execute a request to the IFSC Lookup API.
  ///
  /// Parameters are validated before sending the request.
  Future<IfsclookupResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw IfsclookupException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return IfsclookupResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw IfsclookupException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw IfsclookupException('Resource not found');
      } else {
        throw IfsclookupException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is IfsclookupException) rethrow;
      throw IfsclookupException('Request failed: $e');
    }
  }
...
/// Exception thrown by the IFSC Lookup API client.
class IfsclookupException implements Exception {
  final String message;

  IfsclookupException(this.message);

  @override
  String toString() => 'IfsclookupException: $message';
}
